...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  const handleRegenerate = async () => {
    ...
    // Find the last user message and the corresponding AI response
    let lastUserMessageIndex = -1;
    let lastAIMessageIndex = -1;
    
    for (let i = messages.length - 1; i >= 0; i--) {
      if(lastAIMessageIndex !== -1 && lastUserMessageIndex !== -1) {
        break;
      }
      if (messages[i].sender === 'user' && lastUserMessageIndex === -1) {
        lastUserMessageIndex = i;
      } else if (messages[i].sender === 'ai' && lastAIMessageIndex === -1) {
        lastAIMessageIndex = i;
      }
    }
    
    // If we couldn't find the last user message or AI response, return
    if (lastUserMessageIndex === -1) return;
    console.log('lastAIMessageIndex', lastAIMessageIndex, 'lastUserMessageIndex', lastUserMessageIndex, messages);
    ...
    try {
      ...
      // Create a new AI message with the regenerated response
      const aiMessage: Message = {
        id: lastAIMessageIndex !== -1 ? messages[lastAIMessageIndex].id : (Date.now() + 1).toString(),
        content: response,
        sender: 'ai',
        timestamp: new Date(),
      };
      
      // Update the messages array by replacing the last AI message or adding a new one
      setMessages((prev) => {
        const newMessages = [...prev];
        if (lastAIMessageIndex !== -1) {
          // Replace the existing AI message
      ...
      setMessages((prev) => {
        ...
        if (lastAIMessageIndex !== -1) {
          // Replace the existing AI message
          newMessages[lastAIMessageIndex] = errorMessage;
        }
        return newMessages;
      });
    } finally {
  ...
  // Function to check if a message is the last AI message
  const isLastAIMessage = (message: Message, index: number) => {
    if (message.sender !== 'ai') return false;
    
    // Check if there are any AI messages after this one
    for (let i = index + 1; i < messages.length; i++) {
      if (messages[i].sender === 'ai') return false;
    }
      ...
      <div className="h-80 sm:h-96 overflow-y-auto p-3 sm:p-4 bg-gray-50 dark:bg-gray-900">
        {messages.map((message, index) => (
          ...
          >
            ...
            >
              <div className={`text-sm sm:text-base ${message.sender === 'user' ? '' : 'markdown-content'}`}>
                {message.sender === 'user' ? (
                  <p className="whitespace-pre-line">{message.content}</p>
                ) : (
                  isRegenerating && isLastAIMessage(message, index) ? (
                    <div className="flex items-center space-x-2">
              ...
              <div className="flex justify-between items-center mt-1">
                <p className={`text-xs ${message.sender === 'user' ? 'text-indigo-100' : 'text-gray-400'}`}>
                  {message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </p>
                {isLastAIMessage(message, index) && message.id !== '1' && (
                  <button 
        ...
        {messages.length > 1 && lastUserMessage && (
          <div className="flex justify-end mb-2">
            <button
              ...
              title="Replace the last AI response with a new one"
            >
